        ...
        title: Row(
          children: [
            Container(
              ...
              child: const Icon(Icons.category, color: Color(0xFF1E88E5), size: 22),
            ),
            ...
            onPressed: isSaving
                ...
                : () async {
                    ...
                    try {
                      ...
                      if (existing.any((c) => (c['name'] ?? '').toString().trim().toLowerCase() == name.toLowerCase())) {
                        if (context.mounted) {
                    ...
                    final result = await CatalogService.createCategory(name, desc.isEmpty ? null : desc);
...
void showManageCategoriesDialog(BuildContext context, {VoidCallback? onChanged}) {
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return ListTile(
                                    ...
                                    leading: Container(
                                      ...
                                      child: const Icon(Icons.category, color: Color(0xFF1E88E5), size: 20),
                                    ),
                                    title: Text(
                                      name,
                                    ...
                                    subtitle: Text(
                                      desc.isNotEmpty ? desc : '$count service(s) attached',
                                      style: GoogleFonts.manrope(fontSize: 12, color: Colors.grey[600]),
                                    ),
                                    trailing: Row(
                                      mainAxisSize: MainAxisSize.min,
                                          ...
                                          onPressed: () async {
                                            final confirm = await showDialog<bool>(
                                              ...
                                              builder: (ctx) => AlertDialog(
                                                title: Text('Delete Category', style: GoogleFonts.manrope(fontWeight: FontWeight.bold)),
                                                content: Text('Are you sure you want to delete "$name"? Services under this category will become unassigned.', style: GoogleFonts.manrope()),
                                                actions: [
...
void _showEditCategoryDialog(BuildContext context, Map<String, dynamic> category, VoidCallback onUpdated) {
  ...
  showDialog(
    context: context,
    builder: (ctx) => AlertDialog(
      title: Text('Edit Category', style: GoogleFonts.manrope(fontWeight: FontWeight.bold)),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          TextField(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () async {
            ...
            try {
              final existing = await CatalogService.getAllCategories();
              if (existing.any((c) => (c['name'] ?? '').toString().trim().toLowerCase() == name.toLowerCase() && c['category_id'] != category['category_id'])) {
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('Another category named "$name" already exists.'),
                      backgroundColor: Colors.orange[800],
                    ),
                  );
                }
            ...
            if (result['success'] == true) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(result['message'] ?? 'Failed to update category'), backgroundColor: Colors.red),
              );
            }
          },
